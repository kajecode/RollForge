...
async function main() {
    ...
    const regions = await Regions
        .find({})
        .select("_id name slug type path parent ancestors meta").lean<RegionDoc[]>();
    logger.info(`  Found ${regions.length} regions`);
    logger.info("Fetching Materials");
    const materials = await Materials.find({}).select("_id name slug regions regionSlugs rarity unit unit_weight_lbs unit_price_gp blackmarket variants metadata").lean<MaterialDoc[]>();
    logger.info(`  Found ${materials.length} materials`);
    logger.info("Fetching Items");
    const items = await Items.find({}).select("_id name slug category rarity isMagic basePriceGP priceSource regions materials description shortDescription  blackmarketOnly availabilityBoost tags source notes").lean<ItemDoc[]>();
