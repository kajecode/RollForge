...
export default async function shopsCmd(interaction: ChatInputCommandInteraction) {
  ...
  if (sub === "list") {
    ...
    const rows = await Shop.find(q)
      .sort({ region: 1, town: 1, name: 1 })
      .limit(50)
      .lean<ShopDoc[]>();
    if (!rows.length) {
      await interaction.editReply("No saved shops.");
      return;
    }

    const lines = rows.map(
      (s) =>
        `• **${s.name}** (${s.type}) — ${s.region}${s.town ? ` / ${s.town}` : ""}${s.blackmarket ? " — *blackmarket*" : ""}`,
    );
    const embed = new EmbedBuilder()
      .setTitle("Saved Shops")
  ...
  if (sub === "show") {
    ...
    if (!doc) {
      await interaction.editReply("Shop not found.");
      return;
    }
    ...
    await interaction.editReply({
      files: [file],
      content: `**${name}** (${doc.type}) — ${region}${town ? ` / ${town}` : ""}`,
    });
    return;
  }
}
