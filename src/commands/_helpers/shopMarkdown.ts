export type ShopRenderInput = {
  region: string; town?: string; name: string; type: string;
  locationInTown?: string; proprietor?: string; specialties?: string[];
  inventory: Array<{ name: string; priceGP: number; rarity?: string; isMagic?: boolean; category?: string }>;
...
export function renderShopMarkdown(s: ShopRenderInput): string {
  ...
  const spec = (s.specialties && s.specialties.length)
  ...
  const specials = (s.specialItems && s.specialItems.length)
    ? s.specialItems.map((si, idx) =>
